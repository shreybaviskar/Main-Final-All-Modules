...
class Provider(models.Model):
    ...
    def graph_api_send_template(self, template, language, namespace, partner, params):
        if self.graph_api_authenticated:
            ...
            # ===== ADD THESE LOGS =====
            _logger.info("=== WA TEMPLATE SENDING ===")
            _logger.info("URL: %s", url)
            _logger.info("Partner Mobile: %s", partner.mobile)
            _logger.info("Template: %s | Language: %s", template, language)
            _logger.info("Params/Components: %s", params)
            _logger.info("Full Payload: %s", payload)
            # ==========================
            ...
            try:
                ...
                # ===== ADD THIS =====
                _logger.info("=== META API RESPONSE ===")
                _logger.info("Status Code: %s", answer.status_code)
                _logger.info("Response Body: %s", answer.text)
                # ====================
    ...
    def graph_api_send_template1(self, template, language, namespace, partner, params):
        if self.graph_api_authenticated:
            url = self.graph_api_url + self.graph_api_instance_id + "/messages"
            payload = json.dumps({
                "messaging_product": "whatsapp",
