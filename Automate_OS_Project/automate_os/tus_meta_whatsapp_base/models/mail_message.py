...
class Message(models.Model):
    ...
    def _get_record_name(self, values):
        """Get the display name of the related record"""
        if values.get('model') and values.get('res_id'):
            try:
                record = self.env[values['model']].browse(values['res_id'])
                return record.display_name if record.exists() else False
            except Exception:
                return False
        return False

    def is_thread_message(self, values):
        """Check if this is a thread message (posted on a record)"""
        return values.get('model') and values.get('res_id') and values.get('message_type') != 'user_notification'
    ...
    def create(self, values_list):
        ...
        for values in values_list:
            ...
            if 'body' in values:
                ...
                values['body'] = _image_dataurl.sub(base64_to_boundary, str(values['body']))

            # delegate creation of tracking after the create as sudo to avoid access rights issues
            tracking_values_list.append(values.pop('tracking_value_ids', False))

        messages = super(Message, self.with_context(
            {"is_automated_action": self.env.context.get('is_automated_action', False),
             "report_taken": self.env.context.get('report_taken', False)})).create(values_list)

        check_attachment_access = []
        if all(isinstance(command, int) or command[0] in (4, 6) for values in values_list for command in
               (values.get('attachment_ids') or [])):
            for values in values_list:
                for command in (values.get('attachment_ids') or []):  # Also fix this line
            ...
            check_attachment_access = messages.mapped('attachment_ids').ids
            ...
            for values in values_list:
                if values.get('message_type') == 'wa_msgs':
                    ...
                    vals = {
                        ...
                        'model': self.env.context.get('active_model', 'discuss.channel'),
                    ...
                    if user.id != self.env.ref('base.public_user').id:
                        ...
                        if self.env.context.get('template_send'):
                            if self.env.context.get('active_model_id'):
                                vals.update({'rec_id': self.env.context.get('active_model_id')})
