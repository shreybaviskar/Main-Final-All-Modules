...
class Provider(models.Model):
    ...
    def get_channel_whatsapp(self, partner, user):
        ...
        if provider_channel_id:
            ...
            if channel:
                if len(channel) > 1:
                    channel = channel.sorted('create_date', reverse=True)
                    channel = channel[0] if channel else False
            if user.partner_id.id not in channel.channel_partner_ids.ids and user.has_group(
                    "base.group_user") and user.has_group("aos_whatsapp_base_aretx.whatsapp_group_user"):
