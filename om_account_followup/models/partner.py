...
class ResPartner(models.Model):
    ...
    # added for contact search
    followup_level_code = fields.Integer(
        string="Follow-up Level",
        compute="_compute_followup_level_code",
        store=True,
        index=True,
    )

    @api.depends('latest_followup_level_id')
    def _compute_followup_level_code(self):
        for partner in self:
            # latest_followup_level_id exists logically but is not a model
            partner.followup_level_code = (
                partner.latest_followup_level_id.sequence
